...
public class TypedListTypeAdapter implements InstanceCreator<TypedList>, JsonSerializer<TypedList> {

	@SuppressWarnings({ "unchecked" })
	@Override
	public TypedList createInstance(Type type) {
		if(type instanceof ParameterizedType) {
			ParameterizedType param = (ParameterizedType)type;
			Type[] typeArgs = param.getActualTypeArguments();
			if(typeArgs.length == 0)
				return new TypedList(Object.class);
			if(typeArgs.length != 1)
				throw new IllegalArgumentException("Too many type arguments to TypedList");
			return new TypedList(getRawType(typeArgs[0]));
